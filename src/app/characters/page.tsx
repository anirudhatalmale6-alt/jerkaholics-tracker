...
interface CharacterData {
  ...
  description: string;
  type: string;
  firstAppearanceId: string | null;
  roles: Array<{ id: string; roleName: string }>;
  traits: Array<{ id: string; trait: string }>;
  assets: Array<{
    id: string;
    name: string;
    type: string;
...
export default function CharactersPage() {
  ...
  const { data: charsResp, loading } = useApi<{ data: CharacterData[] }>('/api/characters');

  const characters = charsResp?.data || [];
  const mainCast = useMemo(() => characters.filter(c => c.type === 'main'), [characters]);
  const recurring = useMemo(() => characters.filter(c => c.type === 'recurring'), [characters]);

  const activeCharId = selectedCharId || characters[0]?.id;
  const selectedChar = characters.find(c => c.id === activeCharId);
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-sm" style={{ color: 'var(--muted)' }}>Loading characters...</div>
        </div>
      </>
    );
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            {mainCast.map(char => (
              <button
                ...
                className={`glass-card rounded-xl p-5 text-left transition-all cursor-pointer ${activeCharId === char.id ? 'ring-2 ring-[#8b5cf6]' : 'hover:border-[#3f3f46]'}`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] mt-1" style={{ color: 'var(--muted)' }}>
                      {char.roles.length > 1 ? `${char.roles.length} roles` : char.roles[0]?.roleName || char.name}
                    </div>
                  </div>
                </div>
              </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {recurring.map(char => (
              <button
                key={char.id}
                onClick={() => setSelectedCharId(char.id)}
                className={`glass-card rounded-xl p-3 text-left transition-all cursor-pointer ${activeCharId === char.id ? 'ring-2 ring-[#8b5cf6]' : 'hover:border-[#3f3f46]'}`}
              >
              ...
              <div className="glass-card rounded-xl p-6">
                <div className="flex items-start gap-5 mb-5">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs mt-1" style={{ color: 'var(--muted)' }}>
                      {selectedChar._count.appearances} episode appearance{selectedChar._count.appearances !== 1 ? 's' : ''}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {selectedChar.traits.map(t => (
                    <span key={t.id} className="text-[10px] px-2.5 py-1 rounded-full" style={{ background: 'rgba(139,92,246,0.1)', color: 'var(--accent)', border: '1px solid rgba(139,92,246,0.2)' }}>
                      {t.trait}
                    </span>
                  ))}
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedChar.roles.map(role => (
                        <span key={role.id} className="text-xs px-3 py-1.5 rounded-lg text-[#d4d4d8]" style={{ background: 'rgba(24,24,31,0.8)', border: '1px solid var(--card-border)' }}>
                          {role.roleName}
                        </span>
                      ))}
                    </div>
                        ...
                        {selectedChar.assets.map(asset => (
                          <tr key={asset.id} className="table-row">
                            <td className="px-5 py-3">
                              ...
                              <div className="text-[10px]" style={{ color: 'var(--muted)' }}>{asset.uploadedBy?.name}</div>
                            </td>
                            <td className="px-5 py-3">
                              <span className="text-[10px] px-2 py-0.5 rounded" style={{ background: 'rgba(139,92,246,0.1)', color: 'var(--accent)' }}>
                                {ASSET_TYPE_LABELS[asset.type] || asset.category || asset.type}
                              </span>
                            </td>
            ...
            <div className="space-y-6">
              <div className="glass-card rounded-xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between py-2 border-b" style={{ borderColor: 'var(--card-border)' }}>
                    <span className="text-xs" style={{ color: 'var(--muted)' }}>Appearances</span>
                    <span className="text-xs text-white">{selectedChar._count.appearances}</span>
                  </div>
              ...
              <div className="glass-card rounded-xl p-5">
                ...
                <div className="space-y-2">
                  {characters.filter(c => c.voiceActor).map(c => (
