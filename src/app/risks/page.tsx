      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          {RISKS.sort((a, b) => {
            ...
            const sevColors = SEVERITY_COLORS[risk.severity];
            const statColors = STATUS_STYLES[risk.status];
            return (
              <div
                key={risk.id}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>{risk.id}</span>
                    <span
                ...
                <div className="p-3 rounded-lg" style={{ background: 'rgba(15,15,19,0.5)' }}>
                  ...
                  <p className="text-xs text-[#d4d4d8]">{risk.mitigation}</p>
                </div>

                <div className="flex items-center gap-4 mt-3 text-[10px]" style={{ color: 'var(--muted)' }}>
                  <span>Owner: <span className="text-[#d4d4d8]">{risk.owner}</span></span>
